...
class TestController extends GetxController {
  ...
  findPosition(RxList<ScanResult> deviceList, double angle) async {
    final deviceMap = {
      for (var e in deviceList) e.device.platformName: e,
    };

    ScanResult? topLeftDevice = deviceMap['Ruuvi 862F'];
    ScanResult? topRightDevice = deviceMap['Ruuvi 2559'];
    ScanResult? bottomLeftDevice = deviceMap['Ruuvi B69D'];
    ScanResult? bottomRightDevice = deviceMap['Ruuvi BAAD'];

    getRssiList(
      rssiList: topLeftRssiList,
      device: topLeftDevice,
    );
    getRssiList(
      rssiList: topRightRssiList,
      device: topRightDevice,
    );
    getRssiList(
      rssiList: bottomLeftRssiList,
      device: bottomLeftDevice,
    );
    getRssiList(
      rssiList: bottomRightRssiList,
      device: bottomRightDevice,
    );

    topLeftRssiList = kalmanFilter(topLeftRssiList);
    topRightRssiList = kalmanFilter(topRightRssiList);
    bottomLeftRssiList = kalmanFilter(bottomLeftRssiList);
    bottomRightRssiList = kalmanFilter(bottomRightRssiList);
    ...
    double x3 = 0.0, y3 = 3.0, d3 = topLeftDistance;
    double x4 = 3.6, y4 = 3.6, d4 = topRightDistance;
